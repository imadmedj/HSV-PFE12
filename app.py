"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  DASHBOARD HSV — CÔTES ALGÉRIENNES  (VERSION FUSIONNÉE + CORRECTIFS)        ║
║  Base    : app(12) — toutes fonctionnalités complètes                        ║
║  Correctifs : timeout CMEMS 90s · retry wind_speed 429 · sleep Marine API   ║
...
def _init_download():
    ...
    for folder_path, folder_id in DATA_FOLDERS.items():
        ...
        if not files_exist:
            ...
            print(f"✅ {folder_path} déjà présent ({len(os.listdir(folder_path))} fichiers)")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.spinner(T("loading_map")):
        ...
        df_map = q(f"""
            SELECT NOM_PLAGE, NOM_WILAYA, FIRST(X) AS lon, FIRST(Y) AS lat,
    ...
    if not df_map.empty:
        ...
        if df_map.empty:
            ...
            fig = px.scatter_mapbox(df_map,lat="lat",lon="lon",color="val",size="_size",hover_name="NOM_PLAGE",
                hover_data=hf_clean,color_continuous_scale=cscale,size_max=22,zoom=5,
                mapbox_style="carto-darkmatter",labels={"val":label,"_size":""})
            apply_theme(fig)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if pred_wants_m2 and not is_m2:
        ...
        <div class="incompat-banner">
            ...
            mais le modèle de données actif est <strong>🔵 M1 — ERA5 seul</strong>.</div>
            <div class="ib-step">👉 <strong>Solution :</strong> Sélectionnez <strong>🟣 M2 — ERA5 + CMEMS (1999–2023)</strong> dans la sidebar.</div>
        </div>""", unsafe_allow_html=True)
        st.stop()
        ...
        <div class="incompat-banner">
            ...
            mais le modèle de données actif est <strong>🟣 M2 — ERA5 + CMEMS</strong>.</div>
            <div class="ib-step">👉 <strong>Solution :</strong> Sélectionnez <strong>🔵 M1 — ERA5 seul (1985–2023)</strong> dans la sidebar.</div>
        </div>""", unsafe_allow_html=True)
        st.stop()

    elif pred_wants_m2 and is_m2:
        # ══════════════════════════════════════════════════════════════════════
        # PRÉDICTION M2 — VERSION CORRIGÉE
        # Correctifs : timeout CMEMS 90s · retry wind_speed 429 · sleep Marine
        # ══════════════════════════════════════════════════════════════════════
        ...
        WINDOW_DAYS_M2 = 30
        HORIZONS_M2    = [1, 7]
        FEATURES_DESSAL_M2 = ["wind_speed","mwp","salinity","spm","hour_sin","hour_cos","month_sin","month_cos","x_norm","y_norm"]
        ...
        _missing_m2 = [p for p in [PATH_DESSAL_MODEL,PATH_AQUA_MODEL,PATH_SCALER_DESSAL,PATH_SCALER_AQUA] if not os.path.exists(p)]
        if _missing_m2:
        ...
        # ── FETCH ERA5 / Open-Meteo fallback — CORRIGÉ ───────────────────────
        def _fetch_era5_openmeteo_fallback_m2(lat, lon, date_start, date_end):
            ...
            # ── Helper avec timeout court ────────────────────────────────────
            def _marine_var(var_api, col_out, default_val):
                _time_module.sleep(1)   # ← délai anti-rate-limit
                url = (f"https://marine-api.open-meteo.com/v1/marine?"
                       f"latitude={lat:.4f}&longitude={lon:.4f}&hourly={var_api}"
                       f"&start_date={start_str}&end_date={end_str}&timezone=UTC")
                try:
                    with _ur.urlopen(url, timeout=15) as r:  # ← timeout 15s
                        d = _js.loads(r.read())
                    h = d["hourly"]; n = len(h["time"])
                    df_h = pd.DataFrame({
                        "DATETIME": pd.to_datetime(h["time"]),
            ...
            if "sst" not in df_sst.columns or df_sst["sst"].isna().all():
                ...
                try:
                    ...
                    with _ur.urlopen(url_t2m, timeout=15) as r:
                        d = _js.loads(r.read())
                    h = d["hourly"]; n = len(h["time"])
                    df_t2 = pd.DataFrame({
                        "DATETIME": pd.to_datetime(h["time"]),
            ...
            # ── wind_speed — CORRIGÉ : retry + backoff sur 429 ───────────────
            ...
            url_atm = (f"{base_url_atm}?latitude={lat:.4f}&longitude={lon:.4f}"
                       f"&hourly=wind_speed_10m&start_date={start_str}&end_date={end_str}"
            ...
            for _attempt in range(3):
                try:
                    if _attempt > 0:
                        wait_s = 2 ** _attempt
                        st.caption(f"⏳ Retry wind_speed ({_attempt}/2) dans {wait_s}s...")
                        _time_module.sleep(wait_s)
                    else:
                        _time_module.sleep(1)
                    with _ur.urlopen(url_atm, timeout=15) as r:  # ← timeout 15s
                        d = _js.loads(r.read())
                    h = d["hourly"]; n = len(h["time"])
                    df_atm = pd.DataFrame({
                        "DATETIME"  : pd.to_datetime(h["time"]),
                    ...
                    if _attempt == 2:
                        st.warning(f"⚠️ wind_speed indisponible après 3 tentatives ({e}) → défaut 5.0 m/s")
                        df_result["wind_speed"] = 5.0

            # ── Valeurs par défaut et clipping ───────────────────────────────
            df_result["mwp"]        = df_result["mwp"].fillna(4.0)
            df_result["sst"]        = df_result["sst"].fillna(20.0)
            df_result["wind_speed"] = df_result.get("wind_speed", pd.Series(5.0, index=df_result.index)).fillna(5.0)
        ...
        def _fetch_era5_daily(lat, lon, date_start, date_end, cds_key=""):
            ...
            try:
                ...
                c.retrieve("reanalysis-era5-single-levels",{
                    ...
                    "year":[str(y) for y in years],"month":[str(m).zfill(2) for m in months],"day":days,
        ...
        # ── FETCH CMEMS — CORRIGÉ : timeout 90s via ThreadPoolExecutor ────────
        def _fetch_cmems_daily(lat, lon, date_start, date_end, cmems_user, cmems_pass, variables):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    st.warning("⚠️ CMEMS timeout (>90s) → valeurs par défaut utilisées")
                    return pd.DataFrame()
                except Exception as e:
                    st.warning(f"⚠️ CMEMS erreur : {e} → valeurs par défaut utilisées")
                    return pd.DataFrame()
        ...
        with col_cfg2:
            ...
            pred_date_m2 = st.date_input("📅 Date cible",
            ...
            st.markdown(f"<div style='background:#1e293b;border-left:4px solid #8b5cf6;padding:8px 12px;border-radius:6px;font-size:.85rem;'>⚠️ Seuil de danger HSV : <b style='color:#f59e0b'>{seuil_actif} m</b></div>",unsafe_allow_html=True)
        ...
        if run_m2:
            ...
            try:
                ...
                if is_dessal_m2:
                    features_m2   = FEATURES_DESSAL_M2; app_label_m2 = "Dessalement SWRO"
                    cmems_vars_m2 = ["salinity","spm"]; load_model_fn = _load_model_dessal
                    load_scaler_fn = _load_sc_dessal;   seuil_m2 = SEUIL_HSV_DESSAL
                else:
                    features_m2   = FEATURES_AQUA_M2; app_label_m2 = "Aquaculture Marine"
                    cmems_vars_m2 = ["o2"];           load_model_fn = _load_model_aqua
                    load_scaler_fn = _load_sc_aqua;   seuil_m2 = SEUIL_HSV_AQUA

                win_end = pred_dt_m2; win_start = win_end - timedelta(days=WINDOW_DAYS_M2)

                stat.info(f"⚙️ Chargement modèle {app_label_m2}..."); prog.progress(5)
                ...
                stat.info("🌊 Récupération HSV — Open-Meteo Marine..."); prog.progress(15)
                df_hsv_m2 = _fetch_hsv_openmeteo(lat_m2, lon_m2, win_start, win_end)

                stat.info("🌬️ Récupération ERA5 (wind_speed, MWP, SST)..."); prog.progress(30)
                ...
                if cmems_user_m2.strip() and cmems_pass_m2.strip():
                    stat.info(f"🌊 CMEMS : {', '.join(cmems_vars_m2)} (timeout 90s)..."); prog.progress(48)
                    try:
                        df_cmems_m2 = _fetch_cmems_daily(lat_m2, lon_m2, win_start, win_end,
                                                          cmems_user_m2, cmems_pass_m2, cmems_vars_m2)
                ...
                stat.info("🔧 Construction features M2..."); prog.progress(62)
                ...
                stat.info("📐 Préparation tenseur (1 × 30j × features)..."); prog.progress(74)
                X_m2 = _prepare_tensor_m2(df_feat_m2, features_m2, scaler_m2)

                stat.info(f"🧠 Inférence LSTM {app_label_m2}..."); prog.progress(87)
                ...
                with col_res2:
                    st.markdown(f"<span style='background:#7c3aed;color:white;padding:4px 12px;border-radius:12px;font-size:.82em;'>🟣 M2 — {app_label_m2} · {model_info_m2}</span>",unsafe_allow_html=True)
                    st.markdown("<br>",unsafe_allow_html=True)
                    st.success(f"✅ {sel_plage_m2} · {pred_date_m2.strftime('%d/%m/%Y')}")
                    col_j1, col_j7 = st.columns(2)
                    dt1 = pred_dt_m2 + timedelta(days=1); dt7 = pred_dt_m2 + timedelta(days=7)
                    with col_j1:
                        st.metric(f"HSV J+1 · {dt1.strftime('%d/%m')}", f"{pred_j1:.2f} m")
                        st.markdown(f"<div style='text-align:center;font-weight:700;color:{c1};font-size:1.1rem;'>{lbl1}</div><div style='text-align:center;font-size:.78rem;color:#94b8cc;margin-top:4px;'>Seuil : <b style='color:{c1}'>{seuil_m2} m</b></div>",unsafe_allow_html=True)
                    with col_j7:
                        st.metric(f"HSV J+7 · {dt7.strftime('%d/%m')}", f"{pred_j7:.2f} m", delta=f"{pred_j7-pred_j1:+.2f} m vs J+1")
                        st.markdown(f"<div style='text-align:center;font-weight:700;color:{c7};font-size:1.1rem;'>{lbl7}</div><div style='text-align:center;font-size:.78rem;color:#94b8cc;margin-top:4px;'>Seuil : <b style='color:{c7}'>{seuil_m2} m</b></div>",unsafe_allow_html=True)

                    fig_m2 = go.Figure()
                    fig_m2.add_trace(go.Scatter(x=[dt1,dt7],y=[pred_j1,pred_j7],mode="lines+markers",
                        line=dict(width=4,color="#8b5cf6",dash="dash"),
                        marker=dict(size=16,color=[c1,c7],line=dict(width=2,color="white")),name="HSV prédit"))
                    ...
                    apply_theme(fig_m2)
                    fig_m2.update_layout(title=f"Prévision HSV — {app_label_m2} · {sel_plage_m2}",
                    ...
                    st.dataframe(pd.DataFrame({
                        "Horizon":[" J+1","J+7"],"Date":[dt1.strftime("%d/%m/%Y"),dt7.strftime("%d/%m/%Y")],
                    ...
                    with st.expander("⚙️ Détails techniques M2"):
                        era5_status  = f"✅ {len(df_era5_m2)} jours" if df_era5_m2 is not None and not df_era5_m2.empty else "⚠️ Non disponible"
                        cmems_status = f"✅ {len(df_cmems_m2)} jours" if not df_cmems_m2.empty else "⚠️ Non disponible → valeurs par défaut"
                        st.markdown(
                            f"**Modèle** : {model_info_m2}  \n**Application** : {app_label_m2}  \n"
                            f"**Seuil HSV** : {seuil_m2} m  \n**Features** ({len(features_m2)}) : `{'`, `'.join(features_m2)}`  \n"
                            f"**Fenêtre** : {WINDOW_DAYS_M2} jours  \n**Points contexte** : {len(df_feat_m2)} jours  \n---  \n"
                            f"**Sources données** :  \n• Open-Meteo Marine → HSV : ✅ {len(df_hsv_m2)} jours  \n"
        ...
        try:
            ...
            st.error(f"❌ Dépendances manquantes : {_e}"); st.stop()
        ...
        def _fetch_openmeteo_forecast(lat,lon,date_start,date_end):
            ...
            try:
                with _urllib_request.urlopen(url,timeout=20) as resp:
        ...
        def _build_features_m1(df,lat,lon):
            df=df.copy()
        ...
        def _prepare_window(df,scaler):
            ...
            if len(X)<WINDOW:
                X=np.vstack([np.tile(X[0],(WINDOW-len(X),1)),X])
            X=X[-WINDOW:]
            return X.reshape(1,WINDOW,N_FEAT)

        def _physical_calibration(preds,df_feat,pred_dt,seuil_danger):
            preds=np.array(preds,dtype=float); n=len(df_feat)
            month=pred_dt.month; wind_series=df_feat["wind_speed"].values
            ...
            for idx,(h_horizon,w_size) in enumerate(zip(HORIZONS_M1,[1,6,12])):
                ...
                mean_wind=float(sub["wind_speed"].mean()); max_hsv=float(sub["MESURE"].max())
        ...
        with col_cfg:
            ...
            if not plage_names:
                st.error("❌ Aucune plage disponible dans le dataset M1."); st.stop()
            ...
            st.markdown("---")
            today=datetime.utcnow().date(); era5_cutoff=(datetime.utcnow()-timedelta(days=60)).date()
            ...
            else:                         st.success(f"🚀 **Mode forecast** · Open-Meteo Marine"); use_forecast=True
            st.markdown("---")
        ...
        with col_res:
            section("📊","Résultats"); results_area=st.empty()

        if run_btn:
            if not selected_plage:
                st.error("❌ Veuillez sélectionner une plage."); st.stop()
            try:
                ...
                if len(df_raw)<10:
                    st.error(f"❌ Données insuffisantes : {len(df_raw)} pas de temps."); st.stop()
                for src in sources_used: st.caption(src)
                status.info("🔧 Construction features V7..."); progress.progress(62)
                df_feat=_build_features_m1(df_raw,lat,lon)
...
if is_m2:
    footer_content = (f"Système HSV · Côtes Algériennes · ERA5 + CMEMS (M2) · 1999–2023 &nbsp;·&nbsp; "
                      f"LSTM + Transfer Learning · <strong>2 horizons : +1j · +7j</strong> &nbsp;·&nbsp; "
                      f"DuckDB + Streamlit + Plotly &nbsp;·&nbsp; Copernicus CDS ERA5 · CMEMS · Open-Meteo Marine &nbsp;·&nbsp; 🌐 {lang_info}")
else:
    footer_content = (f"Système HSV · Côtes Algériennes · ERA5 (M1) · 1985–2023 &nbsp;·&nbsp; "
                      f"LSTM + Transfer Learning · <strong>3 horizons : +1h · +6h · +12h</strong> &nbsp;·&nbsp; "
                      f"DuckDB + Streamlit + Plotly &nbsp;·&nbsp; Copernicus CDS ERA5 · Open-Meteo Marine &nbsp;·&nbsp; 🌐 {lang_info}")
st.markdown(f"""<div style="text-align:center;padding:1rem 0;font-size:.75rem;color:var(--text-m);">{footer_content}</div>""",unsafe_allow_html=True)
